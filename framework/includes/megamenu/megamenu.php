...
if ( !class_exists( 'Casano_MegaMenu' ) ) {
	class Casano_MegaMenu
	{
		...
		function register_scripts( $hook )
		{
			...
			wp_enqueue_style( 'casano-megamenu', get_template_directory_uri() . '/framework/includes/megamenu/css/megamenu.min.css' );
			...
			wp_enqueue_media();
			wp_enqueue_script( 'casano-megamenu', get_template_directory_uri() . '/framework/includes/megamenu/js/megamenu.js', array( 'jquery' ), '1.0.0', true );
			...
			wp_localize_script( 'casano-megamenu', 'casano_ajax_backend', array(
					'ajaxurl'  => esc_url( admin_url( 'admin-ajax.php' ) ),
					'security' => wp_create_nonce( 'casano_ajax_backend' ),
				)
			);
		}
		...
		public function set_fonts()
		{
			$this->lits_fonts = array(
				array(
					'title' => esc_html__( 'Font Awesome', 'casano' ),
					'fonts' => array('flaticon-magnifying-glass',
						'flaticon-profile',
						'flaticon-bag',
						...
						'flaticon-tick',
						'flaticon-close',
						'flaticon-circular-check-button',
						'flaticon-check',
		...
		public function addShortcodesCustomCss_megamenu()
		{
			...
			if ( $posts_array ) {
				...
				foreach ( $posts_array as $post ) {
					...
					$shortcodes_custom_css .= get_post_meta( $post->ID, '_casano_shortcode_custom_css', true );
				...
				if ( !empty( $shortcodes_custom_css ) ) {
					...
					wp_add_inline_style( 'casano_custom_css', $shortcodes_custom_css );
				}
			}
		}
