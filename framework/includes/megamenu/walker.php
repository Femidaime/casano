<?php
/**
 * Class Name: casano_bootstrap_navwalker
 ...
 */
if ( ! class_exists( 'Casano_navwalker' ) ) {
	class Casano_navwalker extends Walker_Nav_Menu {
		/**
				...
				if ( $this->megamenu_item_icon_type != "none" ) {
					...
					if ( $this->megamenu_item_icon_type == "image" ) {
						if ( $this->megamenu_img_icon ) {
							...
							$imgicon = casano_resize_image( $this->megamenu_img_icon, 'null', 9000, 9000, false, true, false );
							$imgicon = casano_img_output( $imgicon, 'image_icon_1' );
...
//							}
							...
							if ( $imgicon ) {
								...
								if ( $imgicon_ext == 'svg' ) {
									...
									$item_output .= '<span class="logo">' . casano_img_output( $imgicon_arg, 'image_icon_1' ) . '</span>';
								} else {
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				$fb_output .= '>';
				$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">' . esc_html__( 'Add a menu', 'casano' ) . '</a></li>';
				$fb_output .= '</ul>';
				if ( $container ) {
